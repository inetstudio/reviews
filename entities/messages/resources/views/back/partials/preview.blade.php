@if ($item->hasMedia('files'))
    ...
    @foreach ($media as $mediaItem)
        @if (in_array($mediaItem->mime_type, ['image/jpeg', 'image/png']))
            <a data-fancybox="carousel-{{ $item['id'] }}"
               href="{{ url($mediaItem->getUrl()) }}" {!! (! $loop->first) ? 'style="display: none"' : '' !!}>
                <img src="{{ url($mediaItem->getUrl('review_admin_'.$conversion)) }}" class=" m-b-md img-fluid"
                     alt="post_image">
            </a>
        @else
            @php
            ...
            @endphp
            <a data-fancybox="carousel-{{ $item['id'] }}" data-width="640" data-height="360"
               href="{{ url($mediaItem->getUrl()) }}">
